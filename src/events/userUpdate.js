...
module.exports = async function(oldUser, newUser) {
    ...
    if (newUser.username !== oldUser.username) {
        if (oldUser.username.includes(config.registration.GuilDTag) && !newUser.username.includes(config.registration.GuilDTag)) {
            ...
            client.channels.cache.get(config.channels.chat).send(`${config.emojis.tada} Tebrikler, ${newUser} \`${config.registration.GuilDTag}\` tag alarak ailemize katıldı. Toplam ${guild.members.cache.filter(u => u.user.username.includes(config.registration.GuilDTag)).size} taglı üyemiz var!`)
