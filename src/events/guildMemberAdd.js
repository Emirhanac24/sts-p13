...
module.exports = async member => {
    let tjail = db.get(`sjail_${member.id}`) || 0;
        let tmute = db.get(`scmute_${member.id}`) || 0;
        let twarn = db.get(`smute_${member.id}`) || 0;
    let blk = db.get(`blk_${member.id}`)
    let jail = db.get(`jail_${member.id}`)
    if(jail == true){
        db.set(`roles.${member.id}`, member.roles.cache.map(x => x.id))
        db.set(`isim.${member.id}`, member.displayName)
        db.set(`jail_${member.id}`, true)
        member.setNickname(`[JAİLED-K0] ${member.displayName}`)
        member.roles.set([config.penals.jail.roles])
    member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member} Jail cezası varken sunucudan çıkmış tekrar jail atıldı`) }
    else if(blk == true){
        db.set(`roles.${member.id}`, member.roles.cache.map(x => x.id))
        db.set(`isim.${member.id}`, member.displayName)
        db.set(`jail_${member.id}`, true)
        member.setNickname(`[BLACKLİST] ${member.displayName}`)
        member.roles.set([config.penals.jail.roles])
    member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member} Blacklist cezası olduğu için jail atıldı`) 
    } else {
...
Sunucumuzda toplam **${tjail}** Jail **${tmute}** Mute **${twarn}** Warn cezası almışsın
