...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        db.push(`blacklist_${member.id}`, `${author} tarafından ${moment(Date.now()).format("LLL")} tarihinde **${reason}** sebebiyle blacklist atıldı.`)
        db.push(`sicil_${member.id}`, `${author} tarafından ${moment(Date.now()).format("LLL")} tarihinde **${reason}** sebebiyle blacklist atıldı.`)
        db.add(`blacklis_${member.id}`, 1)
        db.set(`blk_${member.id}`, true)
        db.add(`blacklistveren_${author.id}`, 1)
        db.add(`ceza_${guild.id}`, 1)
        message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı **"${reason}"** sebebiyle ${author} tarafından blacklist atıldı. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)] }).catch((err) => console.log(err), client.ytick(message))
        const user = client.users.cache.get(member)
        client.channels.cache.get(config.penals.warn.log).send({ embeds: [embed.setDescription(`${member} kullanıcısı ${message.author} tarafından blacklist atıldı.
