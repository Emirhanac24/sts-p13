...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!member) return message.reply({ embeds: [embed.setDescription("Öncelikle kullanıcıyı belirtmelisin.")] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        ...
        db.add(`sjail_${member.id}`, 1)
        db.add(`vjail_${guild.id}`, 1)
        ...
        db.push(`sicil_${member.id}`, `${author} Tarafından ${moment(Date.now()).format("LLL")} tarihinde **${reason}** sebebiyle **[ JAİL ]** cezası almış.`)
