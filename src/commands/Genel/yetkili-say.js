...
module.exports = {
    ...
    run: async (client, message, args) => {
      if(message.member.hasPermission('ADMINISTRATOR')) {
            ...
            if(qqq == "etiket"){
            let sesdedeğil = message.guild.members.cache.filter(x => x.roles.cache.has("913377933924651028")).filter(y => !y.voice.channel&& y.presence.status!="offline")
    ...
    ${sesdedeğil.map(s => `${s} \`${s.user.tag}\``).join('\n')}`)
        ...
        if(qqq == "dm"){
          let kullanıcı = message.guild.members.cache.filter(s => s.roles.cache.has("913377933924651028")).filter(s => !s.voice.channel).size
        for(var i = 0;i < kullanıcı;i++){
          let a = message.guild.members.cache.filter(s => s.roles.cache.has("913377933924651028")).filter(s => !s.voice.channel).map(a => a)[i]
          const userDM = await a.createDM()
        try {
          await userDM.send(`<@${a.user.id}> Sese girer misin? `)
        } catch {
